...
class TestDbFuncUlid:
    ...
    def test_generate(self, fx_db_client_tmp_migrated: DatabaseClient):
        """ULID can be generated as a UUID."""
        result = fx_db_client_tmp_migrated.get_query_result(SQL("""SELECT generate_ulid() as id;"""))
        assert len(result) == 1
        assert isinstance(result[0][0], UUID)

        ulid_parse(result[0][0])

    def test_parse(self, fx_db_client_tmp_migrated: DatabaseClient):
        """External ULID can be parsed."""
        result = fx_db_client_tmp_migrated.get_query_result(
            SQL("""SELECT parse_ulid('01HYR9NC4WSVF2ZR0AHQCV1SE8') as id;""")
        )
        assert len(result) == 1

    def test_ulid_to_uuid(self, fx_db_client_tmp_migrated: DatabaseClient):
    ...
    def test_uuid_to_ulid(self, fx_db_client_tmp_migrated: DatabaseClient):
...
class TestDbFuncDdm:
    ...
    def test_format(self, fx_db_client_tmp_migrated: DatabaseClient):
...
class TestDbFuncLabelsValidity:
    ...
    def test_valid(self, fx_db_client_tmp_migrated: DatabaseClient):
        """Valid labels can be inserted."""
        fx_db_client_tmp_migrated.insert_dict(
            "public",
            "asset",
            {
                "labels": Jsonb(
    ...
    def test_invalid_wrapper_not_obj(self, fx_db_client_tmp_migrated: DatabaseClient):
    ...
    def test_invalid_wrapper_no_version(self, fx_db_client_tmp_migrated: DatabaseClient):
    ...
    def test_invalid_wrapper_invalid_version(self, fx_db_client_tmp_migrated: DatabaseClient):
    ...
    def test_invalid_wrapper_no_values(self, fx_db_client_tmp_migrated: DatabaseClient):
    ...
    def test_invalid_values_not_list(self, fx_db_client_tmp_migrated: DatabaseClient):
    ...
    def test_invalid_items_not_objects(self, fx_db_client_tmp_migrated: DatabaseClient):
    ...
    def test_invalid_missing_required_scheme(self, fx_db_client_tmp_migrated: DatabaseClient):
    ...
    def test_invalid_missing_pref_labels(self, fx_db_client_tmp_migrated: DatabaseClient):
    ...
    def test_invalid_multiple_pref_labels(self, fx_db_client_tmp_migrated: DatabaseClient):
