...
creation_time = datetime.datetime(2012, 6, 10, 14, 30, 20, tzinfo=datetime.timezone.utc)
...
class TestGeotabProvider:
    ...
    def test_init_error_config(self, config: GeotabConfig):
    ...
    def test_init_error_geotab(
        self, mocker: MockerFixture, fx_provider_geotab_config: GeotabConfig, fx_logger: logging.Logger
    ):
    ...
    @pytest.mark.vcr
    def test_fetch_devices(self, fx_provider_geotab: GeotabProvider):
        assert list(fx_provider_geotab._fetch_devices())[0] == {
            "device_id": "b8",
    ...
    @pytest.mark.vcr
    def test_fetch_device_statuses(self, fx_provider_geotab: GeotabProvider):
        assert list(fx_provider_geotab._fetch_device_statuses())[0] == {
            "bearing_degrees": 132.0,
            "date": datetime.datetime(2024, 7, 6, 13, 49, 47, tzinfo=datetime.timezone.utc),
            "device_id": "b7",
            "latitude": 56.0257034,
    ...
    @pytest.mark.vcr
    def test_fetch_log_record(self, fx_provider_geotab: GeotabProvider):
        assert fx_provider_geotab._fetch_log_record(
            device_id="b7", time=datetime.datetime(2024, 7, 6, 13, 49, 47, tzinfo=datetime.timezone.utc)
        ) == {
            "dateTime": datetime.datetime(2024, 7, 6, 13, 49, 47, tzinfo=datetime.timezone.utc),
            "device": {"id": "b7"},
            "id": "b114521",
            "latitude": 56.0257034,
            "longitude": -3.44706559,
    ...
    def test_fetch_log_record_error(
        ...
        device_id = "b7"
        time = datetime.datetime(2024, 7, 6, 13, 49, 47, tzinfo=datetime.timezone.utc)
    ...
    def test_fetch_log_record_none(
        ...
        with pytest.raises(IndexError, match="No log record found"):
            client._fetch_log_record(device_id="x", time=datetime.datetime.now(tz=datetime.timezone.utc))
    ...
    @pytest.mark.cov
    def test_index_assets(self, fx_provider_geotab_mocked: GeotabProvider):
        key = "y"
        assets = [
            Asset(
                id=new_ulid(),
    ...
    @pytest.mark.vcr
    def test_fetch_active_assets(self, freezer: FrozenDateTimeFactory, fx_provider_geotab: GeotabProvider):
        ...
        assert list(fx_provider_geotab.fetch_active_assets())[0] == expected_asset

    def test_fetch_active_assets_error(
        self, caplog: pytest.LogCaptureFixture, mocker: MockerFixture, fx_provider_geotab_mocked: GeotabProvider
    ):
    ...
    @pytest.mark.vcr
    def test_fetch_latest_positions(self, freezer: FrozenDateTimeFactory, fx_provider_geotab: GeotabProvider):
        freezer.move_to(creation_time)
        ...
        expected_position = PositionNew(
            asset_id=asset.id,
            time=datetime.datetime(2024, 7, 6, 18, 12, 58, tzinfo=datetime.timezone.utc),
    ...
    def test_fetch_latest_positions_unknown_heading(
        self, freezer: FrozenDateTimeFactory, mocker: MockerFixture, fx_provider_geotab_mocked: GeotabProvider
    ):
        freezer.move_to(creation_time)
        ...
        # can't use next() here as at least one call to each generator method must get all items to avoid pytest-cov
        # flagging the generator loop has not completed
        position = list(fx_provider_geotab_mocked.fetch_latest_positions(assets=[asset]))[0]
