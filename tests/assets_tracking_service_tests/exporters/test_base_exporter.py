...
class TestBaseExporter:
    ...
    def test_init(self: Self, fx_config: Config, fx_db_client_tmp_db_pop: DatabaseClient, fx_logger: logging.Logger):
        """Initialises."""
        exporter = ExampleExporter(config=fx_config, db=fx_db_client_tmp_db_pop, logger=fx_logger)
        assert exporter is not None

    def test_export(self: Self, fx_exporter_example: ExampleExporter):
        """Exports."""
