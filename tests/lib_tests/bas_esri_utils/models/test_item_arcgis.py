...
from assets_tracking_service.lib.bas_data_catalogue.models.record.elements.identification import (
    GraphicOverview as RGraphicOverview,
)
...
class TestItemArcGIS:
    ...
    def test_item_type(self, fx_lib_record_minimal_iso: Record):
        """Can get the ArcGIS Item type/resource."""
        expected = ItemTypeEnum.GEOJSON
        item = Item(fx_lib_record_minimal_iso, arcgis_item_type=expected, arcgis_item_name="x")

        assert item.item_type == expected

    def test_item_name(self, fx_lib_record_minimal_iso: Record):
        """Can get the ArcGIS Item service name."""
        expected = "x"
        item = Item(fx_lib_record_minimal_iso, arcgis_item_type=ItemTypeEnum.GEOJSON, arcgis_item_name=expected)

        assert item.item_name == expected

    @pytest.mark.cov()
    def test_title(self, fx_lib_record_minimal_iso: Record):
        """Can format title without Markdown."""
        fx_lib_record_minimal_iso.identification.title = "_x_"
        item = Item(fx_lib_record_minimal_iso, arcgis_item_type=ItemTypeEnum.GEOJSON, arcgis_item_name="...")

        assert item.title_plain == "x"

    @pytest.mark.parametrize(("lineage", "citation", "href"), [(None, None, None), ("-y-", "-z-", "-l-")])
    def test_description(
        self, fx_lib_record_minimal_iso: Record, lineage: str | None, citation: str | None, href: str | None
    ):
        """Can render description from template."""
        abstract = "-x-"

        fx_lib_record_minimal_iso.identification.abstract = abstract
        if citation is not None:
            fx_lib_record_minimal_iso.identification.other_citation_details = citation
        if href is not None:
            fx_lib_record_minimal_iso.identification.identifiers = [
                Identifier(RIdentifier(identifier="x", href=href, namespace="data.bas.ac.uk"))
            ]
        if lineage is not None:
            fx_lib_record_minimal_iso.data_quality = DataQuality(lineage=Lineage(statement=lineage))
    ...
    def test_terms_of_use(self, fx_lib_record_minimal_iso: Record, value: str | None, expected: str | None):
    ...
    def test_terms_of_use_unknown(self, fx_lib_record_minimal_iso: Record):
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                "x",
                [RIdentifier(identifier="x", href="x", namespace="data.bas.ac.uk")],
                [
                    RConstraint(
    ...
    def test_item_properties(
        self,
        fx_lib_record_minimal_iso: Record,
        purpose: str | None,
        identifiers: list[RIdentifier] | None,
        constraints: list[RConstraint] | None,
        lineage: str | None,
    ):
        """Can get combined ArcGIS item properties."""
        expected = "x"
        item_type = ItemTypeEnum.GEOJSON
        fx_lib_record_minimal_iso.identification.title = expected
        fx_lib_record_minimal_iso.identification.abstract = expected
        if purpose is not None:
            fx_lib_record_minimal_iso.identification.purpose = purpose
        if identifiers is not None:
            fx_lib_record_minimal_iso.identification.identifiers = identifiers
        if constraints is not None:
            fx_lib_record_minimal_iso.identification.constraints = constraints
        if lineage is not None:
            fx_lib_record_minimal_iso.data_quality = DataQuality(lineage=Lineage(statement=lineage))
        item = Item(fx_lib_record_minimal_iso, arcgis_item_type=item_type, arcgis_item_name=expected)
    ...
    @pytest.mark.parametrize(
        ("value", "expected"),
        [
            (RConstraint(type=ConstraintTypeCode.ACCESS), SharingLevel.PRIVATE),
            (
    ...
    def test_sharing_level(self, fx_lib_record_minimal_iso: Record, value: RConstraint | None, expected: SharingLevel):
    ...
    def test_sharing_level_override(self, fx_lib_record_minimal_iso: Record):
        """Can override the ArcGIS sharing level."""
        value = AccessType.BAS_ALL
        expected = SharingLevel.ORG
        item = Item(
            fx_lib_record_minimal_iso, arcgis_item_type=ItemTypeEnum.GEOJSON, arcgis_item_name="...", access_type=value
        )

        assert item.sharing_level == expected

    def test_metadata(self, fx_lib_record_minimal_iso: Record):
        """Can get ArcGIS item metadata."""
        fx_lib_record_minimal_iso.file_identifier = "x"
        expected = f"<mdFileID>{fx_lib_record_minimal_iso.file_identifier}</mdFileID>"
        item = Item(fx_lib_record_minimal_iso, arcgis_item_type=ItemTypeEnum.GEOJSON, arcgis_item_name="...")

        assert expected in item.metadata

    @pytest.mark.parametrize("value", [[RGraphicOverview(identifier="overview-agol", href="x", mime_type="x")], None])
    def test_thumbnail_href(self, fx_lib_record_minimal_iso: Record, value: list[RGraphicOverview] | None):
