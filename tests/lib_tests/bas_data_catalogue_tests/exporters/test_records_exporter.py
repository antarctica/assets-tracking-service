...
class TestRecordsExporter:
    ...
    def test_loads(self, fx_lib_exporter_records: RecordsExporter, fx_lib_record_minimal_item_catalogue: Record):
    ...
    def test_get_exporters(
        ...
        result = fx_lib_exporter_records_pop._get_exporters(fx_lib_record_minimal_item_catalogue)
    ...
    def test_export_record(
        ...
        site_path = fx_lib_exporter_records_pop._config.EXPORTER_DATA_CATALOGUE_OUTPUT_PATH
        ...
        fx_lib_exporter_records_pop._records[record_id].identification.identifiers.append(
            Identifier(
                identifier="datasets/y", href="https://data.bas.ac.uk/datasets/y", namespace="alias.data.bas.ac.uk"
            )
        )

        fx_lib_exporter_records_pop.export_record(fx_lib_record_minimal_item_catalogue.file_identifier)

        result = list(fx_lib_exporter_records_pop._config.EXPORTER_DATA_CATALOGUE_OUTPUT_PATH.glob("**/*.*"))
        for path in expected:
            assert path in result

    def test_publish_record(
        self,
        fx_lib_exporter_records_pop: RecordsExporter,
        fx_lib_record_minimal_item_catalogue: Record,
        fx_s3_bucket_name: str,
    ):
        ...
        record_id = fx_lib_record_minimal_item_catalogue.file_identifier
        s3_utils = S3Utils(
            logger=fx_lib_exporter_records_pop._logger,
            s3=fx_lib_exporter_records_pop._s3_client,
            s3_bucket=fx_s3_bucket_name,
            relative_base=fx_lib_exporter_records_pop._config.EXPORTER_DATA_CATALOGUE_OUTPUT_PATH,
        )
        expected = [
            "items/x/index.html",
        ...
        fx_lib_exporter_records_pop.publish_record(fx_lib_record_minimal_item_catalogue.file_identifier)
    ...
    def test_export_all(self, fx_lib_exporter_records_pop: RecordsExporter):
        """Can export all records."""
        fx_lib_exporter_records_pop.export()

        result = list(fx_lib_exporter_records_pop._config.EXPORTER_DATA_CATALOGUE_OUTPUT_PATH.glob("**/*.*"))
        assert len(result) > 0

    def test_publish_all(self, fx_lib_exporter_records_pop: RecordsExporter):
        """Can export all records."""
        bucket = fx_lib_exporter_records_pop._config.EXPORTER_DATA_CATALOGUE_AWS_S3_BUCKET
        s3_utils = S3Utils(
            logger=fx_lib_exporter_records_pop._logger,
            s3=fx_lib_exporter_records_pop._s3_client,
            s3_bucket=bucket,
        ...
        fx_lib_exporter_records_pop.publish()
