...
from assets_tracking_service.lib.bas_data_catalogue.models.record import Identification
...
class TestRecordMagicDiscoveryV1:
    ...
    def test_loads(self, fx_lib_record_config_minimal_magic_preset: dict):
        """Can create a minimal Record from a JSON serialised dict."""
        record = RecordMagicDiscoveryV1.loads(fx_lib_record_config_minimal_magic_preset)
        assert isinstance(record, RecordMagicDiscoveryV1)

    def test_metadata_contact(self, fx_lib_record_config_minimal_magic_preset: dict):
    ...
    def test_catalogue_identifier(self, fx_lib_record_config_minimal_magic_preset: dict):
    ...
    def test_catalogue_identifier_existing(self, fx_lib_record_config_minimal_magic_preset: dict):
        """Does not include a duplicate catalogue identifier if already in record."""
        expected = Identifier(identifier="x", href="https://data.bas.ac.uk/items/x", namespace="data.bas.ac.uk")
        # noinspection PyTypeChecker
        fx_lib_record_config_minimal_magic_preset["identification"]["identifiers"] = [asdict(expected)]
        record = RecordMagicDiscoveryV1.loads(fx_lib_record_config_minimal_magic_preset)

        matches = [i for i in record.identification.identifiers if i == expected]
        assert len(matches) == 1

    def test_poc(self, fx_lib_record_config_minimal_magic_preset: dict):
    ...
    def test_poc_existing(self, fx_lib_record_config_minimal_magic_preset: dict, contacts: list[dict]):
        """Includes MAGIC as a point of contact."""
        fx_lib_record_config_minimal_magic_preset["identification"]["contacts"] = contacts
        record = RecordMagicDiscoveryV1.loads(fx_lib_record_config_minimal_magic_preset)
    ...
    def test_profile(self, fx_lib_record_config_minimal_magic_preset: dict):
        """Includes domain consistency element for profile."""
        record = RecordMagicDiscoveryV1.loads(fx_lib_record_config_minimal_magic_preset)
        assert EXPECTED_PROFILE in record.data_quality.domain_consistency

    def test_catalogue_profile_existing(self, fx_lib_record_config_minimal_magic_preset: dict):
        ...
        fx_lib_record_config_minimal_magic_preset["identification"]["domain_consistency"] = [profile]
        record = RecordMagicDiscoveryV1.loads(fx_lib_record_config_minimal_magic_preset)

        matches = [p for p in record.data_quality.domain_consistency if p == EXPECTED_PROFILE]
        assert len(matches) == 1

    def test_lineage_statement(self, fx_lib_record_config_minimal_magic_preset: dict):
        """Can set a lineage statement."""
        expected = "x"
        fx_lib_record_config_minimal_magic_preset["identification"]["lineage"] = {"statement": expected}
        record = RecordMagicDiscoveryV1.loads(fx_lib_record_config_minimal_magic_preset)
