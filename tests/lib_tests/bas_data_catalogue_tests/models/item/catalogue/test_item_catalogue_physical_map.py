...
from assets_tracking_service.lib.bas_data_catalogue.models.item.catalogue import Dates, Identifiers
...
class TestItemCataloguePhysicalMap:
    ...
    def test_init(self, fx_config: Config, fx_lib_record_minimal_item_catalogue_physical_map: Record):
        ...
        assert item._record == fx_lib_record_minimal_item_catalogue_physical_map
    ...
    def test_matches(
        ...
        record = fx_lib_record_minimal_item_catalogue_physical_map if matches else fx_lib_record_minimal_item_catalogue
        assert ItemCataloguePhysicalMap.matches(record) == matches

    def test_sides(self, fx_config: Config, fx_lib_item_catalogue_min_physical_map: ItemCataloguePhysicalMap):
        """Can get records representing the sides of a physical map."""
        expected = [
            _lib_get_record(side.identifier.identifier)
            for side in fx_lib_item_catalogue_min_physical_map._record.identification.aggregations.filter(
                associations=AggregationAssociationCode.IS_COMPOSED_OF,
                initiatives=AggregationInitiativeCode.PAPER_MAP,
            )
        ]

        assert fx_lib_item_catalogue_min_physical_map._sides == expected

    def test_tabs(self, fx_lib_item_catalogue_min_physical_map: ItemCataloguePhysicalMap):
        """Can get physical map specific tabs."""
        assert isinstance(fx_lib_item_catalogue_min_physical_map._extent, ExtentTab)
        assert isinstance(fx_lib_item_catalogue_min_physical_map._additional_info, AdditionalInfoTab)

    @staticmethod
    def _get_record_graphics(identifier: str) -> Record:
        """Extension of `_lib_get_record` to include graphic overviews."""
        record = _lib_get_record(identifier=identifier)
    ...
    def test_graphics(
        ...
        fx_lib_record_minimal_item_catalogue_physical_map.identification.graphic_overviews.append(
    ...
    def test_side_summaries(self, fx_lib_item_catalogue_min_physical_map: ItemCataloguePhysicalMap):
        """Can get ItemSummaries for each map side."""
        sides = fx_lib_item_catalogue_min_physical_map.sides
        assert len(sides) > 0
