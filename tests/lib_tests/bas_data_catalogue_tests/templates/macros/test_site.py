...
class TestMacrosSite:
    ...
    def test_head_meta(self, fx_lib_item_catalogue: ItemCatalogue):
        """
    ...
    def test_head_favicon(self, fx_lib_item_catalogue: ItemCatalogue):
        """Can get static favicon."""
        html = BeautifulSoup(fx_lib_item_catalogue.render(), parser="html.parser", features="lxml")
        assert html.head.find(name="link", rel="icon") is not None

    def test_head_title(self, fx_lib_item_catalogue: ItemCatalogue):
        """Can get <title> with expected value from item."""
        html = BeautifulSoup(fx_lib_item_catalogue.render(), parser="html.parser", features="lxml")
        assert html.head.title.string == fx_lib_item_catalogue.html_title

    @pytest.mark.parametrize(
        "href", ["https://cdn.web.bas.ac.uk/libs/font-awesome-pro/5.13.0/css/all.min.css", "/static/css/main.css"]
    )
    def test_head_styles(self, fx_lib_item_catalogue: ItemCatalogue, href: str):
        """Can get static CSS references."""
        html = BeautifulSoup(fx_lib_item_catalogue.render(), parser="html.parser", features="lxml")
        assert html.head.find(name="link", rel="stylesheet", href=href) is not None

    def test_head_open_graph(self, fx_lib_item_catalogue: ItemCatalogue):
        """
        ...
        """
        html = BeautifulSoup(fx_lib_item_catalogue.render(), parser="html.parser", features="lxml")
        for key, val in fx_lib_item_catalogue.html_open_graph.items():
            assert html.head.find(name="meta", property=key)["content"] == val

    def test_head_schema_org(self, fx_lib_item_catalogue: ItemCatalogue):
        """
